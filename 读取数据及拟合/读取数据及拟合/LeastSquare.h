...
private:
	...
	*/
	...
	SCoordinate SphericalToCartesian(SCoordinate point);
	SCoordinate xyzToXY(SCoordinate point);
	double sigma(vector<double>Array,int size);//数组求和
	void get_data(string OriginalDatasPath);
	vector<double> Gauss();//列主元高斯消去
public:
	LeastSquare();
	~LeastSquare();
	...
	void setCoordinate(SOriginalDatas inputOriginalDatas);
	void SphericalToCartesian(vector<double> Azimuth, vector<double>Elevation, vector<double> Range);//球坐标转笛卡尔坐标x,y,z
	

	void fitting(string OriginalDatasPath = "", int N = 2);
	void fitting(SOriginalDatas inputOriginalDatas);
	vector<double> fitting(vector<double>x, vector<double>y, int N);
	void CartesianToSpherical(double X, double Y, double Z);//笛卡尔坐标转球坐标
	SCoordinate CartesianToSpherical(SCoordinate point);
